# add-notification-system-fixed.ps1
...
# Also skips hidden/system directories (.git, .github, .vscode, etc.)
...
# We use exactly the same formatting as index.html
$headerNotifIcon = @'
...
foreach ($filePath in $allHtmlFiles) {
    ...
    try {
        ...
        # 1. Check/Add CSS link before </head>
        if (-not $content.Contains("notification-popup.css")) {
        ...
        # 2. Check/Add notification bell icon as first item in <ul class="headIc">
        #    Note: There is only one <ul class="headIc"> in all files, representing the header icons list.
        if (-not $content.Contains("notification-icon")) {
            if ($content.Contains('<ul class="headIc">')) {
                $content = $content.Replace('<ul class="headIc">', '<ul class="headIc">' + $headerNotifIcon)
                $modified = $true
            }
        }
        
        # 3. Check/Add JS script and popup HTML before </body>
        if (-not $content.Contains("notification-popup.js")) {
...
Write-Host "Total HTML files processed: $totalFiles"
Write-Host "Files modified:            $modifiedFiles"
Write-Host "Files already complete:    $skippedAlreadyHasAll"
Write-Host "Files excluded:            $skippedExcluded"
