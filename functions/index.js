...
async function sendDigestAlerts(frequency) {
    ...
    try {
        await db.collection("adminReports").add({
            type: "digest_alert",
            reportPeriod: frequency,   // "daily" | "weekly" | "monthly"
            frequency,
            usersProcessed,
            totalQueuedJobsSent,
            totalExpiredQueuedJobsRemoved,
...
function buildAdminReportHTML(periodLabel, jobAlerts, digestAlerts) {
    ...
    if (jobAlerts.length > 0) {
        jobAlerts.forEach((job) => {
            ...
            html += `
                <div style="margin-bottom:25px;border:1px solid #ddd;padding:15px;border-radius:8px;">
                    ...
                    <p><strong>Instant matched users:</strong> ${job.instantMatchedUsersCount ?? 0}</p>
                    <p><strong>Queued users (daily/weekly/monthly):</strong> ${job.queuedUsersCount ?? 0}</p>
                    <p><strong>Total instant emails sent:</strong> ${job.emailsSentCount ?? 0}</p>
                    <p><strong>Instant push notifications queued (staggered):</strong> ${job.pushQueuedCount ?? 0}</p>
        ...
        html += `<p>No instant job alerts triggered during this period.</p>`
    }
